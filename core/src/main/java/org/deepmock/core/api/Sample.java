 ...
 * to reproduce the Sample by their own means (i.e. due to changes in the underlying database, or change of time, etc.), the methods can be coerced to reproduce the Sample using
 * this API.
 *
 ...
 *
 * DeepSampler defines Samples on classes rather then on objects. This enables DeepSampler to change the behavior of all instances of these classes without the need to create the
 * sampled instances manually and to distribute the Sampler into the objects that will be tested. The distribution is done by a Dependency Injection Framework like Spring or Guice.
 *
